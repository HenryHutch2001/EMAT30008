...
# %%
D = 1 # Defining the diffusion coefficient
...
""" for n in range(0,N_t):
    for i in range(0,N-1):
        if i == 0:
            u[n+1,0] = u[n,0] + C * (alpha-2*u[n,0]+u[n,1]) + dt*source(x_int[i],t[n],u[n,i])
        if 0 < i and i < N-2:
            u[n+1,i] = u[n,i] + C * (u[n,i+1] - 2 * u[n,i] + u[n,i-1]) + dt*source(x_int[i],t[n],u[n,i])
        else:
            u[n+1,N-2] = u[n, N-2] + C * (beta - 2 * u[n, N-2] + u[n, N-3]) + dt*source(x_int[i],t[n],u[n,i]) 
 """
for n in range(0,N_t):
    for i in range(0,N-1):
        if i == 0:
            u[n+1,0] = u[n,0] + C * (alpha-2*u[n,0]+u[n,1]) + dt*source(x_int[i],t[n],u[n,0])
        elif i < N-2:
            u[n+1,i] = u[n,i] + C * (u[n,i+1] - 2 * u[n,i] + u[n,i-1]) + dt*source(x_int[i],t[n],u[n,i])
        else:
            u[n+1,N-2] = u[n, N-2] + C * (beta - 2 * u[n, N-2] + u[n, N-3]) + dt*source(x_int[N-3],t[n],u[n,N-2])
...
def ExplicitEuler(N,domain_start,domain_end,D,t_final,dt,bc_left,bc_right,initial_condition,source,*args):
    ...
    t = dt * np.arange(N_t) 
    u = np.zeros((N_t+1,N-1))
    u[0,:] = initial_condition(x_int,a,b)
    for n in range(0,N_t):
        for i in range(0,N-1):
            if i == 0:
...
x,y = ExplicitEuler(22,0,1,1,1,0.00001,0.0,0.0,f,source)
plt.plot(x,y)
plt.show()
# %%
